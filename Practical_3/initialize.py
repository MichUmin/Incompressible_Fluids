...
def velocity(velocity_field, mesh):
    ...
    for index in range(N):
        velocity_field.values[index] = [0.0, 1.0, 0.0]
    velocity_field.boundary_values = {'left': [0.0, 1.0, 0.0], 'right': [0.0, 1.0, 0.0], 'top': [0.0, 1.0, 0.0], 'bottom': [0.0, 1.0, 0.0], 'do_nothing': [0.0, 0.0, 0.0]}
...
def diffusion_coef(gamma_vector, mesh):
    N = len(mesh.cells)
    for index in range(N):
        gamma_vector.values[index] = [0.001]
    gamma_vector.boundary_values =  {'left': 0.001, 'right': 0.001, 'top': 0.001, 'bottom': 0.001, 'do_nothing': 0.0}
